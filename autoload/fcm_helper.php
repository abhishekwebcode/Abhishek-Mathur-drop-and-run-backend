 ...
 * @param array $body
*/
function pushToOne($fcm_id,$title,$message,$body) {
    $push=new PushNotification(API_KEY);
    $push->
    to($fcm_id)
        ->withTitle($title)
        ->withMessage($message)
 ...
 * @throws
*/
function pushToMany($fcm_ids,$title,$message,$body) {
    $push=new PushNotification(API_KEY);
...
function pushDataToMany($fcm_ids,$data) {
    $push=new PushNotification(API_KEY);
    foreach ($fcm_ids as $fcm_id) {
        $push->to($fcm_id)
            ->withTitle($title)
