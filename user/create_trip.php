 ...
 * */
$check = select("SELECT * FROM drivers WHERE online='online' AND max_weight >= ? ", array("s", $trip_weight));
if (sizeof($check) == 0) {
    appError(
        false,
...
VALUES (?,?,?,?,?,?,?,?,?,?,?)"
        , array("sssssssssii", $rec_phone, $phone_of_user, $trip_pickup_latitude, $trip_pickup_longitude, $trip_drop_latitude, $trip_drop_longitude, TRIP_STATUSES::$TRIP_CREATED, strval(gettimeofday()["sec"]), $trip_type_car, (int)$trip_weight, (int)$trip_pin)
    );
    $newTripid = $mysqli->insert_id;
    $fcm_ids_of_drivers = array();
    foreach ($check as $item) {
        $fcm_ids_of_drivers[] = $item["fcm_id"];
    }
    ...
    basic_fcm(array("to" => "/topics/driver", "data" =>
        array_merge(
            array("trip_id" => $newTripid, "new_booking" => true), $input
        )
    ));
    appError(true, "Your Booking has been requested", array("trip_id" => $newTripid,"i"=>$input));
}
?>